...
const CSV_HEADERS = ['Date', 'Start Time', 'End Time', 'Court', 'Player', 'Total Amount', 'Deposit Amount', 'Status'];
...
const formatRowLine = (values) =>
  PDF_COLUMNS.map((column, index) => truncate(values[index], column.width).padEnd(column.width)).join(' ');

const bookingToPdfRow = (booking) => formatRowLine([
  formatDate(booking.date),
  booking.startTime,
  booking.endTime,
  booking.court?.name || '',
...
const buildPdfDocument = (lines) => {
  ...
  pageChunks.forEach((chunk, i) => {
    ...
    objects[pageNum] = `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PDF_PAGE_WIDTH} ${PDF_PAGE_HEIGHT}] /Resources << /Font << /F1 ${fontObjNum} 0 R >> >> /Contents ${contentNum} 0 R >>`;
    objects[contentNum] = { stream: buildContentStream(chunk) };
  });

  objects[fontObjNum] = '<< /Type /Font /Subtype /Type1 /BaseFont /Courier /Encoding /WinAnsiEncoding >>';
...
export const generatePdf = (bookings, filters = {}) => {
  ...
  lines.push('-'.repeat(PDF_COLUMNS.reduce((sum, column) => sum + column.width + 1, -1)));
