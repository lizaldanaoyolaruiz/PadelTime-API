...
export const getMantenimientos = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Error al obtener mantenimientos.', error: error.message });
  }
};
...
export const crearMantenimiento = async (req, res) => {
  try {
    ...
    if (!courtId || !date || !startTime || !endTime) {
      return res.status(400).json({ message: 'Faltan campos requeridos: courtId, date, startTime, endTime.' });
    }
    ...
    if (req.user.role === 'admin') {
      const complejo = await Complex.findOne({ _id: cancha.complex, owner: req.user._id });
    ...
    return res.status(500).json({ message: 'Error al crear el mantenimiento.', error: error.message });
  }
};
...
export const eliminarMantenimiento = async (req, res) => {
  try {
    ...
    if (!slot) return res.status(404).json({ message: 'Slot de mantenimiento no encontrado.' });

    if (req.user.role === 'admin') {
      const complejo = await Complex.findOne({ _id: slot.complex, owner: req.user._id });
      if (!complejo) return res.status(403).json({ message: 'Acceso denegado.' });
    }
    ...
    return res.status(500).json({ message: 'Error al eliminar el mantenimiento.', error: error.message });
  }
};
