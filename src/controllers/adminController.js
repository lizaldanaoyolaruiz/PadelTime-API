...
export const toggleAdminStatus = async (req, res) => {
  try {
    ...
    if (!["approved", "suspended"].includes(status)) {
      return res
        .status(400)
        .json({ message: 'Estado inválido. Usar "approved" o "suspended".' });
    }
    ...
    res
      .status(500)
      .json({ message: "Error al cambiar el estado.", error: error.message });
  }
};
...
export const getStats = async (req, res) => {
  try {
    ...
    const [totalComplexes, activeUsers, monthlyReservations] =
      await Promise.all([
