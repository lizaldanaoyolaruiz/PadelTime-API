...
export const checkFavorito = async (req, res) => {
  try {
    const usuario = await User.findById(req.user._id).select('favoritos').lean();
    const esFavorito = (usuario.favoritos || []).some(
      (id) => id.toString() === req.params.complexId
    );
    return res.json({ esFavorito });
  } catch (error) {
    return res.status(500).json({ message: 'Error al verificar favorito.', error: error.message });
  }
};
...
export const agregarFavorito = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Error al agregar favorito.', error: error.message });
  }
};
...
export const quitarFavorito = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Error al quitar favorito.', error: error.message });
  }
};
