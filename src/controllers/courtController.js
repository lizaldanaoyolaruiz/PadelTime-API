...
const getComplexIfOwner = async (complexId, userId, role) => {
  ...
  if (complex.owner.toString() !== userId.toString() && role !== "superadmin")
    return null;
  return complex;
};
  ...
  try {
    ...
    if (req.file) {
      const result = await uploadImage(
        req.file.buffer,
        `padeltime/complexes/${complexId}/courts`,
      );
    ...
    res
      .status(500)
      .json({
        message: "Lo sentimos, no pudimos crear la cancha. Intenta nuevamente.",
        error: error.message,
      });
  }
};
...
export const updateCourt = async (req, res) => {
  try {
    ...
    fields.forEach((f) => {
      if (req.body[f] !== undefined) court[f] = req.body[f];
    });

    if (req.file) {
      const result = await uploadImage(
        req.file.buffer,
        `padeltime/complexes/${court.complex}/courts`,
      );
...
export const uploadCourtPhotos = async (req, res) => {
  try {
    ...
    const uploaded = await Promise.all(
      req.files.map((f) =>
        uploadImage(f.buffer, `padeltime/complexes/${court.complex}/courts`),
      ),
    );
  ...
  try {
    ...
    if (req.user.role === "superadmin") {
      if (!req.query.complexId) {
        return res
          .status(400)
          .json({ message: "Se requiere complexId en query para superadmin" });
      }
      ...
      if (!complexId) {
        ...
        if (!complex)
          return res
            .status(403)
            .json({ message: "No autorizado para este complejo" });
      }
    }
  ...
  try {
    ...
    if (req.user.role === "superadmin") {
      if (!req.body.complexId) {
        return res
          .status(400)
          .json({ message: "Se requiere complexId para superadmin" });
      }
      ...
      if (!complexId) {
        ...
        if (!complex)
          return res
            .status(403)
            .json({ message: "No autorizado para modificar esta cancha" });
      }
    }
    ...
    if (!Array.isArray(days) || days.length === 0) {
      return res
        .status(400)
        .json({ message: "El campo days es requerido y debe ser un arreglo." });
    }
    ...
    res
      .status(500)
      .json({ message: "Error al actualizar horarios.", error: error.message });
  }
};
