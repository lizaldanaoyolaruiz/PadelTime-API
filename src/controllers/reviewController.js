...
export const createReview = async (req, res) => {
  try {
    ...
    if (!complexId || !rating) {
      return res.status(400).json({ message: 'complexId and rating are required.' });
    }
    ...
    if (!booking) {
      return res.status(403).json({
        message: 'Solo podés valorar complejos donde tengas una reserva confirmada.',
      });
    }

    const exists = await Review.findOne({ complex: complexId, user: req.user._id });
    if (exists) {
      return res.status(400).json({ message: 'Ya enviaste una valoración para este complejo.' });
    }
    ...
    res.status(201).json({ message: 'Valoración enviada. ¡Gracias por tu feedback!', review });
  } catch (error) {
    if (error.code === 11000) {
      return res.status(400).json({ message: 'Ya enviaste una valoración para este complejo.' });
    }
...
export const canReview = async (req, res) => {
  try {
    ...
    res.json({ canReview: Boolean(booking) && !existingReview, alreadyReviewed: Boolean(existingReview) });
  } catch (error) {
...
export const updateReview = async (req, res) => {
  try {
    ...
    if (String(review.user) !== String(req.user._id)) {
      return res.status(403).json({ message: 'No tenés permisos para editar esta valoración.' });
    }
...
export const deleteReview = async (req, res) => {
  try {
    ...
    if (!isOwnerOfReview && req.user.role !== 'superadmin') {
      return res.status(403).json({ message: 'No tenés permisos para eliminar esta valoración.' });
    }
