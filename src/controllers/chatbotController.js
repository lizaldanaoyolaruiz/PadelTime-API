...
async function buscarAlternativas({
  ...
  for (let h = openHour; h < closeHour && alternativas.length < 3; h++) {
    ...
    const globallyBlocked = blockouts.some(
      (b) => !b.courtId && slotOverlapsBlockout(slotStart, slotEnd, b),
    );
    ...
    const freeCourts = courts.filter((c) => {
      ...
      return (
        sMin >= timeToMinutes(courtOpen) &&
        eMin <= (timeToMinutes(courtClose) || 24 * 60)
      );
    });
    ...
    if (freeCourts.length > 0) {
      const canchas = freeCourts.map((c) => {
        ...
        const waText = whatsappNum
          ? encodeURIComponent(
              `Hola! Quiero reservar ${c.name} el ${fechaFormateada} de ${slotStart} a ${slotEnd}. ¿Tienen disponibilidad?`,
            )
          : null;
        return {
          ...
          linkWhatsApp: whatsappNum
            ? `https://wa.me/${whatsappNum}?text=${waText}`
            : null,
        };
      });
...
async function executeVerificarDisponibilidad({
  complexId,
  date,
  startTime,
  courtType,
  ...
  try {
    ...
    if (
      date < nowAR.date ||
      (date === nowAR.date && timeToMinutes(startTime) < nowAR.totalMinutes)
    ) {
      return {
        error: "fecha_pasada",
        mensaje:
          "No se pueden verificar ni reservar fechas u horarios ya transcurridos.",
      };
    }
    ...
    const scheduledCourts = courts.filter(
      (c) => c.schedule?.[dayEN]?.enabled !== false,
    );

    const openCourts = scheduledCourts.filter((c) => {
      const courtOpen =
        c.schedule?.[dayEN]?.start || complex.openTime || "00:00";
      const courtClose =
        c.schedule?.[dayEN]?.end || complex.closeTime || "00:00";
    ...
    const alternativasOpciones =
      alternativasSugeridas.length > 0
        ? {
            complejo: complex.name,
            fechaFormateada,
            slots: alternativasSugeridas,
          }
        : null;
    ...
    return {
      ...
      alternativasSugeridas: alternativasSugeridas.map((a) => ({
        horario: a.horario,
        canchasDisponibles: a.canchas.length,
      })),
      ...
      parameters: {
        ...
        properties: {
          complexId: {
            type: "string",
            description:
              'ID del complejo (campo "ID:" del listado del sistema)',
          },
          date: {
            type: "string",
          ...
          courtType: {
            ...
            description:
              'Tipo de cancha preferida. Usar "any" si no especificó.',
          },
        },
...
export const chatbot = async (req, res) => {
  ...
  try {
    ...
    const complejos = await Complex.find({
      status: { $in: ["approved", "pending"] },
    })
      .select(
        "_id name city location price openTime closeTime whatsapp instagram mercadopagoActive depositPercentage",
      )
      .limit(20)
      .lean();
